...
const SECTIONS = [
...

  {
    ...
    subtitle: "From breadboard to finished instrument — soldering, power, debugging, and enclosures",
    concepts: [

      {
        id: "breadboard-to-pcb",
        title: "Breadboard to PCB",
        tags: ["proto"],
        chain: ["Breadboard prototype works", "Schematic captured in software", "Component footprints placed", "Copper traces routed", "Gerber files sent to fab"],
        blurb: "Once your breadboard prototype works, a PCB makes it permanent, reliable, and compact. PCB design tools like KiCad (free) or EasyEDA (browser-based) let you design professional boards. Fabrication is cheap and fast.",
        detail: `<strong>PCB design workflow:</strong>\n1. Schematic — draw your circuit in the schematic editor. Same symbols as paper schematics.\n2. Assign footprints — link each component symbol to its physical footprint (how big the pads are, pin spacing).\n3. PCB layout — place components on the board, route copper traces connecting them.\n4. Design Rule Check (DRC) — software checks for errors: short circuits, traces too close, missing connections.\n5. Generate Gerber files — the manufacturing files every fab accepts.\n6. Order — upload to JLCPCB, PCBWay, or OSHPark. 5 boards for ~$5–10, ships in 1–2 weeks from China.\n\n<strong>PCB layer basics:</strong>\n• Top copper — main component and trace layer\n• Bottom copper — second routing layer\n• Silkscreen — labels, component outlines, pin 1 markers\n• Soldermask — green (or black/blue) protective coating, leaves pads exposed\n\n<strong>Design rules of thumb:</strong>\n• Signal traces: 0.2–0.4 mm wide\n• Power traces: 1 mm per amp (minimum 1 mm for any power rail)\n• Clearance between traces: 0.2 mm minimum\n• Ground plane: fill unused copper areas with GND — reduces noise and provides return path\n\n<strong>Tool recommendations:</strong>\n• EasyEDA — browser-based, easy, integrates with JLCPCB ordering. Best for beginners.\n• KiCad — free, open-source, industry-standard. Steeper learning curve, more powerful.`,
        memory: `PCB design = solving a puzzle. You have component footprints (the pieces) and must connect them with copper traces (wires) without crossing. The software checks your work. Then you mail the puzzle file to a factory and they print it in copper and fiberglass.\n\nStart with EasyEDA. Your first PCB will have mistakes — order 5 copies, fix what's wrong, order again. Iteration is cheap.`,
        examTip: `JLCPCB offers PCB assembly (PCBA) — they solder the SMD components for you. For a simple sensor board with common parts, PCBA can be cheaper and faster than hand-soldering. Use their part library when choosing components to maximize assembly compatibility.`,
        facts: ["KiCad = free, open-source", "EasyEDA = browser-based beginner", "Gerber = manufacturing files", "JLCPCB = cheap fab (~$5 for 5)", "Signal trace: 0.2–0.4mm", "Power trace: ~1mm/A", "Ground plane = noise reduction", "DRC = error check before ordering"]
      },

      {
        ...
        title: "Soldering",
        tags: ["proto"],
        chain: ["Iron heats the joint", "Solder flows into heated junction", "Wets both pad and component lead", "Iron removed", "Joint cools undisturbed"],
        blurb: "Soldering is the foundational skill for permanent electronics. Heat the joint — not the solder. A good joint is shiny, smooth, and cone-shaped. Bad joints are the #1 source of intermittent hardware failures.",
        detail: `<strong>Equipment you need:</strong>\n• Soldering iron: temperature-controlled station preferred. Hakko FX-888D is the gold standard (~$100). Budget option: TS100 or Pinecil. Avoid cheap fixed-temperature irons.\n• Temperature: 315–370°C (600–700°F) for leaded solder. 350–400°C for lead-free.\n• Solder: 60/40 or 63/37 tin/lead rosin-core is easiest to work with. 63/37 is eutectic (no pasty phase — goes directly liquid to solid).\n• Tip cleaner: brass wool (not wet sponge — thermal shock damages tips).\n• Solder wick (desoldering braid): removes excess solder from pads.\n• Flux pen: apply extra flux to difficult joints to help solder flow.\n\n<strong>Technique — the golden rule:</strong>\n1. Clean the tip on brass wool\n2. Tin the tip (coat with fresh solder)\n3. Touch tip to BOTH the pad AND the component lead simultaneously\n4. Hold 1–2 seconds to heat the joint\n5. Touch solder to the JUNCTION (not the iron) — it flows in\n6. Remove solder, then remove iron\n7. Do NOT move the joint for 3–5 seconds while it solidifies\n\n<strong>Good joint:</strong> shiny, smooth, concave cone shape. Wets both the pad and the lead.\n<strong>Cold joint:</strong> dull, grainy, lumpy. Caused by moving before solid or insufficient heat.\n<strong>Solder bridge:</strong> unintended solder blob connecting two adjacent pads. Remove with solder wick.`,
        memory: `Soldering = welding for ants. The one rule: heat the joint, not the solder. The solder goes where it's hot. If you melt the solder on the iron tip and dab it on the pad, you get a cold joint. Heat the pad and the pin together FIRST, then the solder flows in by itself.\n\nShiny = good. Dull/grainy = cold joint = do it again.`,
        examTip: `Cold joints cause intermittent failures — the connection works sometimes, fails other times depending on temperature and vibration. If a circuit works on the bench but fails in the field, suspect cold joints first. Reflow them with fresh flux and heat.`,
        facts: ["Heat the joint, not the solder", "315–370°C iron temp", "Shiny = good joint", "Dull/grainy = cold joint", "63/37 solder = easiest", "Brass wool tip cleaner", "Wick = remove excess solder", "Flux improves flow"]
      },

      {
        ...
        tags: ["proto"],
        chain: ["Power source selected", "Voltage regulated to required level", "Current capacity verified", "Decoupling caps placed", "Protection circuits added"],
        blurb: "Power design is where most beginners go wrong. Wrong voltage = dead components. Insufficient current = mysterious resets. Poor filtering = random glitches. Getting power right is the foundation every other circuit depends on.",
        detail: `<strong>Linear regulators (simple, inefficient):</strong>\n• LM7805: 5V output, up to 1.5A. Needs 7V+ input. Wastes (Vin − 5V) × I as heat.\n• AMS1117-3.3: 3.3V, 1A. Common on dev boards. Low dropout: only needs 1.3V headroom.\n• When to use: low current (<500mA), small voltage drop, simplicity matters.\n• Heat: P_waste = (Vin − Vout) × I. 12V→5V at 500mA = 3.5W heat. Needs a heatsink.\n\n<strong>Buck converters (switching, efficient):</strong>\n• 80–95% efficient — much less heat than linear regulators.\n• Step DOWN voltage only (buck). Step UP = boost converter.\n• Common modules: LM2596 (cheap, 3A), MP2307 (small, 1.2A).\n• When to use: higher currents, large voltage drops, battery-powered projects.\n• Downside: generates electrical noise — add extra decoupling caps near sensitive circuits.\n\n<strong>Building a power budget:</strong>\n1. List every component and its operating current\n2. Sum them: this is your minimum supply current\n3. Add 25–30% safety margin\n4. Choose a supply and regulator that comfortably exceeds this total\n\n<strong>Decoupling capacitors (non-negotiable):</strong>\nPlace a 100nF ceramic + 10µF electrolytic across VCC/GND at every IC power pin. This suppresses high-frequency noise and handles momentary current spikes.\n\n<strong>Protection basics:</strong>\n• Polyfuse (resettable fuse): auto-resets after overcurrent. Put on VCC input.\n• Schottky diode in series: blocks reverse polarity. 1N5819 for low voltage drop.\n• TVS diode: clamps voltage spikes from motors and inductive loads.`,
        memory: `Power system = a city water supply. Reservoir (battery) → treatment plant (regulator) → distribution pipes (traces) → each building (component). Size the pipes for the flow, keep the pressure stable, add shock absorbers near sensitive equipment (decoupling caps).\n\nLinear regulator = simple but wastes energy as heat. Buck converter = efficient but noisier. Pick based on current and voltage drop.`,
        examTip: `The most common power bug: supply voltage sags under load. To test: measure VCC with your multimeter WHILE the system is running at full load. A 5V supply dropping to 4.6V under motor load will cause an ESP32 to brownout-reset. Fix: bigger supply or better regulator.`,
        facts: ["LM7805 = 5V linear", "AMS1117-3.3 = 3.3V linear", "Buck = efficient (80–95%)", "Linear = simple but hot", "100nF + 10µF decoupling at every IC", "Polyfuse = overcurrent protection", "Schottky = reverse polarity", "Power budget = sum + 30% margin"]
      },

      {
        id: "debugging-hardware",
        title: "Debugging Hardware",
        tags: ["proto"],
        chain: ["Something doesn't work", "Check power first", "Check connections second", "Measure with instruments", "Divide and conquer", "Fix one thing at a time"],
        blurb: "Hardware debugging is systematic elimination, not guessing. The bug is almost never where you think it is. Work through a checklist, measure everything, and isolate the problem before changing anything.",
        detail: `<strong>The universal debug order:</strong>\n1. Power — is VCC correct? Is GND connected everywhere? Measure with multimeter.\n2. Visual inspection — solder bridges? Component installed backwards? Wrong pin? Missing wire?\n3. Heat — anything getting hot that shouldn't? Capacitor installed backwards gets hot fast.\n4. Divide and conquer — disconnect parts of the circuit until it works, then reconnect one at a time.\n5. Measure, don't assume — use multimeter and oscilloscope.\n\n<strong>Most common hardware bugs (in order of frequency):</strong>\n• Unconnected GND between two circuits\n• Component installed backwards (LED, electrolytic cap, diode, IC orientation)\n• TX and RX not crossed on UART\n• Missing I2C pull-up resistors\n• Wrong I2C address\n• Floating input pin (no pull-up/pull-down)\n• 5V signal into a 3.3V input\n• Motor noise causing MCU resets (missing decoupling caps)\n• Baud rate mismatch on serial\n• Wrong board/port selected in Arduino IDE before upload\n\n<strong>Serial Monitor is your best friend:</strong>\nAdd Serial.println() everywhere. Print sensor readings, state names, calculated values, timestamps. When you can see what the code thinks is happening, bugs become obvious.\n\n<strong>I2C Scanner:</strong>\nRun the I2C scanner sketch whenever an I2C device isn't responding. It prints every address that responds. Device not found = wrong pins, missing pull-ups, wrong voltage, or wrong address.`,
        memory: `Hardware debugging = detective work. Your first instinct is wrong. The obvious suspect is innocent. Check power first (most failures), then connections (second most common), then code (rarely the problem), then re-read the datasheet (usually reveals what you missed).\n\nThe component is almost never broken. You wired it wrong.`,
        examTip: `Before spending an hour debugging, do the 60-second power check: multimeter to VCC (correct voltage?), multimeter between VCC and GND in continuity mode (should NOT beep — a beep means a short circuit). These two checks catch 40% of hardware bugs instantly.`,
        facts: ["Check power first", "Multimeter always", "No guessing — measure", "TX→RX not TX→TX", "I2C scanner sketch", "Serial.println() = debug", "Divide and conquer", "Read the datasheet again"]
      },

      {
        ...
        tags: ["proto"],
        chain: ["Circuit built", "Need to verify what's happening", "Multimeter measures V/I/R/continuity", "Oscilloscope shows signals over time", "Logic analyzer decodes protocols"],
        blurb: "Test equipment lets you see what's actually happening in your circuit — not what you think is happening. A multimeter solves 80% of problems. An oscilloscope solves most of the rest.",
        detail: `<strong>Multimeter (buy this first, essential):</strong>\n• DC voltage — measure supply rails, sensor outputs, signal levels\n• Continuity mode — beeps when two points are connected. Check solder joints, trace continuity, verify wiring.\n• Resistance — verify resistor values, check if a trace is continuous\n• DC current — measure actual current draw (put meter IN SERIES with the load)\n• Recommended: Fluke 101, Klein MM300, or AstroAI DM6000B (~$20–50)\n\n<strong>Oscilloscope (buy this second, very helpful):</strong>\n• Displays voltage over time — see PWM waveforms, serial data, signal timing, noise\n• Essential for: motor control debugging, serial protocol issues, audio, sensor timing\n• Best value for beginners: Rigol DS1054Z (~$350, 4 channels, 50 MHz, often unlocks to 100 MHz)\n• Budget option: Hantek DSO5102P (~$80, 2 channels)\n• USB oscilloscopes: Digilent Analog Discovery 2 (great for laptops)\n\n<strong>Logic analyzer (helpful for digital protocols):</strong>\n• Captures digital signals and decodes I2C, SPI, UART automatically\n• Shows exactly which bytes were sent and received\n• Cheap option: clone Saleae 24 MHz 8-channel (~$10–15) + free Sigrok/PulseView software\n\n<strong>Bench power supply (highly recommended):</strong>\n• Adjustable voltage and current limit — set max current to protect circuits during testing\n• Current limit triggers before components fry\n• Recommended: Korad KA3005P or RD6006 (~$50–80)`,
        memory: `Multimeter = a magnifying glass (examines static clues). Oscilloscope = a slow-motion camera (captures what happens over time). Logic analyzer = a lip reader for digital signals (tells you exactly what bits were sent).\n\nStart with the multimeter. It solves 80% of problems and costs $20. The oscilloscope solves most of the rest.`,
        examTip: `The current-limiting bench power supply is underrated. Set the current limit to just above your circuit's normal draw before powering up for the first time. If there's a short circuit, the supply limits current instead of vaporizing your traces. An essential safety tool.`,
        facts: ["Multimeter: V/I/R/continuity", "Rigol DS1054Z = best value scope", "Logic analyzer = protocol decode", "Bench supply = adjustable + current limit", "Continuity mode for solder checks", "In-series for current measurement", "PulseView = free logic analyzer software", "Fluke 101 = reliable budget multimeter"]
      },

      {
        id: "enclosures-connectors",
        title: "Enclosures & Connectors",
        tags: ["proto"],
        chain: ["Prototype works on bench", "Must survive real use", "Enclosure protects electronics", "Right connectors prevent failures", "IP rating for wet environments"],
        blurb: "Moving a project off the bench means protecting it. The right enclosure and connectors are the difference between a working instrument and a fragile mess. Loose wires are the most common failure mode in DIY lab hardware.",
        detail: `<strong>Enclosures:</strong>\n• ABS plastic project boxes (Hammond, Bud Industries): standard, cheap, widely available in many sizes.\n• IP ratings: IP65 = dust-tight + water jet resistant. IP67 = submersible to 1m for 30 min. For lab environments with liquids, IP65+ is worth it.\n• 3D printed: custom shapes, PLA for normal use, PETG for mild chemical resistance and higher temperature, ABS for higher temp. Design in FreeCAD or Fusion 360.\n• DIN rail enclosures: for mounting electronics inside electrical panels.\n\n<strong>Connectors — choose carefully:</strong>\n• Dupont/JST-XH: breadboard-style. Easy but not reliable for repeated plugging.\n• JST-GH: smaller, latching, reliable. Used in drones and robotics. Good for sensors.\n• XT30 / XT60: high-current power connectors for LiPo batteries. XT30 = 30A, XT60 = 60A.\n• Barrel jack (5.5mm / 2.1mm): common power input on dev boards.\n• M12 connectors: industrial-grade, IP67, screw-lock. Used for sensors in wet/dirty environments. The right choice for permanent lab installations.\n• Phoenix Contact screw terminals: reliable for permanent wiring of power and signal in enclosures.\n\n<strong>Wire gauge (AWG — lower = thicker):</strong>\n• 22–24 AWG: signal wires, low current\n• 20 AWG: general purpose up to ~1.5A\n• 18 AWG: up to 5A\n• 16 AWG: motors and high-current loads\n• Always add strain relief where cables enter enclosures.`,
        memory: `Connector = a handshake between circuits. The wrong handshake (wrong rating, wrong type) = dropped connection at the worst time. In lab hardware: connectors fail before components. Spend money on good connectors and crimp them properly.\n\nM12 connectors = the ones that screw on and survive splashes. Use them for anything mounted near liquids in a lab.`,
        examTip: `Strain relief is non-negotiable for any cable passing through an enclosure wall. Without it, flexing the cable stresses the solder joint or connector contact directly. A cable tie anchored inside the enclosure just behind the entry point is the minimum. Proper strain relief grips the cable jacket.`,
        facts: ["IP67 = dust + waterproof (1m)", "PETG = chemical-resistant printing", "XT60 = LiPo battery connector", "M12 = industrial IP67 sensor connector", "AWG: lower = thicker", "22AWG = signal, 18AWG = power", "Strain relief always", "JST-GH = reliable robotics connector"]
      },

      {
        id: "lab-automation-hardware",
        title: "Lab Automation Hardware",
        tags: ["proto"],
        chain: ["Biological process to automate", "Sensors measure parameters", "MCU reads and decides", "Actuators control environment", "Data logs over WiFi", "You monitor remotely"],
        blurb: "Everything in this guide connects here: building actual bioinformatics lab hardware. Temperature control, liquid handling, environmental monitoring, and wireless data logging — the practical destination of all these concepts.",
        detail: `<strong>Incubator controller (entry-level lab automation):</strong>\n• Sensor: DS18B20 (temperature) + SHT31 (humidity)\n• Controller: ESP32 (WiFi for data logging)\n• Algorithm: PID controller driving a relay\n• Actuator: relay → heater (NC relay = fail-safe)\n• Safety: hardware over-temperature cutoff (separate from software)\n• Data: MQTT → Node-RED → InfluxDB → Grafana dashboard\n\n<strong>Syringe pump (precise liquid dispensing):</strong>\n• Motor: NEMA17 stepper + A4988 driver\n• Mechanism: leadscrew converts rotation to linear motion\n• Control: count steps → known volume (calibrate steps-per-mL)\n• Interface: buttons + OLED display or serial commands from Pi\n\n<strong>Peristaltic pump system (media/reagent delivery):</strong>\n• Motor: DC gear motor + DRV8833 driver\n• Control: run for timed duration at calibrated speed\n• Upgrade: flow sensor (YF-S201 or Sensirion SLF3S) for closed-loop volume control\n\n<strong>Environmental monitoring station:</strong>\n• Sensors: SHT31 (temp/humidity), BMP280 (pressure), BH1750 (light), CCS811 (CO2)\n• MCU: ESP32\n• Storage: SD card (local) + WiFi upload to database\n• Power: USB or 18650 LiPo with TP4056 charger\n\n<strong>Safety design rules for lab hardware:</strong>\n• Isolate mains voltage with relays and optocouplers — never route 120V through MCU circuitry\n• Hardware over-temperature cutoff (bimetallic thermostat in series with heater) independent of software\n• Fail-safe defaults: heater OFF, pump OFF on MCU crash or sensor failure\n• Chemical compatibility: PETG or polypropylene for parts contacting reagents`,
        memory: `Lab automation = stacking this entire guide. Sensor (reads world) → MCU (thinks) → actuator (acts) → data log (remembers) → alert (notifies you).\n\nA PID-controlled incubator on an ESP32 with WiFi data logging to Grafana is achievable in a few focused weekends. You now know every piece needed to build it.`,
        examTip: `Start with the simplest useful thing: an ESP32 logging temperature and humidity to a Google Sheet or MQTT broker. Once that works reliably, add one actuator (a relay controlling a heater). Master the feedback loop. Then add more sensors. Build incrementally — don't design the full system before you've made one part work.`,
        facts: ["DS18B20 = incubator temp", "PID + relay = temperature control", "NEMA17 + A4988 = syringe pump", "Peristaltic = reagent delivery", "MQTT → Node-RED → Grafana", "ESP32 = wireless lab sensor", "Hardware over-temp cutoff always", "PETG/PP for chemical contact"]
      },
